...
//a reference to a Python exception to be returned on error (m95Exception)
int32_t m95exc;
...
SocketAPIPointers m95_api;
/**
 * @brief Init modem driver, calls _gs_init, gzsock_init
 *
 * As last parameter, requires an integer saved to global m95exc, representing the name assigned to m95Exception
 * so that it can be raised by returning m95exc. If modules initialization is successful, the next step is calling startup
 *
 */
C_NATIVE(_m95_init){
    NATIVE_UNWARN();
    ...
    *res = MAKE_NONE();
    ...
    ACQUIRE_GIL();
    m95_api.socket = m95_gzsock_socket;
    m95_api.connect = m95_gzsock_connect;
    m95_api.setsockopt = m95_gzsock_setsockopt;
    m95_api.getsockopt = m95_gzsock_getsockopt;
    m95_api.send = m95_gzsock_send;
    m95_api.sendto = m95_gzsock_sendto;
    m95_api.write = m95_gzsock_write;
    m95_api.recv = m95_gzsock_recv;
    m95_api.recvfrom = m95_gzsock_recvfrom;
    m95_api.read = m95_gzsock_read;
    m95_api.close = m95_gzsock_close;
    m95_api.shutdown = m95_gzsock_shutdown;
    m95_api.bind = m95_gzsock_bind;
    m95_api.accept = NULL;
    m95_api.listen = NULL;
    m95_api.select = m95_gzsock_select;
    m95_api.fcntl = m95_gzsock_fcntl;
    m95_api.ioctl = NULL;
    m95_api.getaddrinfo = m95_gzsock_getaddrinfo;
    m95_api.freeaddrinfo = m95_gzsock_freeaddrinfo;
    m95_api.inet_addr = NULL;
    m95_api.inet_ntoa = NULL;
...
/**
 * @brief Setup modem serial port, AT configuration and start modem thread (used at power on)
 *
 */
...
/**
 * @brief Stop modem thread and close serial port (used at power off)
 *
 */
...
/**
 * @brief _m95_detach removes the link with the APN while keeping connected to the GSM network
 *
 *
 */
...
/**
 * @brief _m95_attach tries to link to the given APN
 *
...
C_NATIVE(_m95_attach){
    ...
    *res = MAKE_NONE();
    ...
    //Pre-condition is "modem ready" (will do only once)
    if (!gs.modem_ready) {
        // wait for the modem to be "ready" or +CGATT=1 will fail the first time
        // 3 seconds ok, 2 seconds not, let's be conservative and wait 4 seconds
        vosThSleep(TIME_U(4000,MILLIS));
        gs.modem_ready = 1;
    }
...
/**
 * @brief _m95_operators retrieve the operator list and converts it to a tuple
 *
 *
 */
...
/**
 * @brief _m95_set_operator try to set the current operator given its name
 *
 *
 */
...
/**
 * @brief _m95_rssi return the signal strength as reported by +CIEV urc
 *
 *
 */
C_NATIVE(_m95_rssi){
...
/**
 * @brief strings for network types (see bits in GS_RAT_xxx)
 */
const uint8_t * const _urats[] = { "","GSM","GPRS","GSM+GPRS" };
#define MAX_URATS (sizeof(_urats)/sizeof(*_urats))

/**
 * @brief _m95_network_info retrieves network information through +URAT and *CGED
 *
 *
 */
C_NATIVE(_m95_network_info){
    ...
    // get RAT (radio access technology) string from static buffer
    if (gs.tech < 0 || gs.tech >= MAX_URATS) p0=0;
    else p0=gs.tech;
    str = pstring_new(strlen(_urats[p0]),_urats[p0]);
...
/**
 * @brief _m95_mobile_info retrieves info on IMEI and SIM card by means of +CGSN and *CCID
 *
 *
 */
...
/**
 * @brief _m95_link_info retrieves ip and dns by means of +UPSND
 *
 *
 */
...
C_NATIVE(_m95_socket_recv_into){
    C_NATIVE_UNWARN();
    uint8_t* buf;
    int32_t len;
    int32_t sz;
...
C_NATIVE(_m95_socket_recvfrom_into){
    C_NATIVE_UNWARN();
    uint8_t* buf;
    int32_t len;
    int32_t sz;
