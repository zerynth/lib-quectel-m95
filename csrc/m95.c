/**
 * @file m95.c
 * @brief Driver for Quectel M95 modules
...
/**
 * @brief Initializes the data structures of m95
 */
void _gs_init(void)
{
    int i;
...
/**
 * @brief Checks if gs.buffer contains "n, CLOSED\r\n" URC
 *
 * @return 0 on failure
 */
int _gs_check_closed_urc(void)
{
    if (gs.bytes < 11 || gs.buffer[0]<'0' || gs.buffer[0]>'9')
        return 0;
    uint8_t *p = (uint8_t*)memchr(gs.buffer, ',', 3);
    return p != 0 && memcmp(p, ", CLOSED\r\n", 10) == 0;
}
/**
 * @brief Checks if gs.buffer contains "+PDP DEACT\r\n" URC
 *
 * @return 0 on failure
 */
int _gs_check_pdpdeact_urc(void)
{
    return gs.bytes >= 12 && memcmp(gs.buffer, "+PDP DEACT\r\n", 12) == 0;
}
...
int _gs_config0(void)
{
    ...
    _gs_send_at(GS_CMD_QENG, "=i,i", 1, 0);
    if (!_gs_wait_for_ok(500))
        return 0;
   ...
   for (i = 50; i > 0; --i) {
    ...
    _gs_send_at(GS_CMD_CGEREP, "=i", 1);
    if (!_gs_wait_for_ok(2000))
        return 0;

    //fix socket close behavior
    vhalSerialWrite(gs.serial, "AT+QCFG=\"TCP/BufferRemain\",1\r\n", 30);
    _gs_wait_for_ok(500); // allow errors for old fw rev

    _gs_send_at(GS_CMD_QIMUX, "=1");
    if (!_gs_wait_for_ok(500))
        return 0;

    _gs_send_at(GS_CMD_QINDI, "=1");
    if (!_gs_wait_for_ok(500))
        return 0;
...
void _gs_handle_urc(GSCmd* cmd)
{
    ...
    case GS_CMD_QSSLOPEN:
        ...
        break;
    case GS_CMD_QIRDI:
        nargs = _gs_parse_command_arguments(buf, ebuf, "iii", &p0, &p1, &p2);
        if (nargs != 3)
            goto exit_err;
        _gs_socket_pending(p2);
        break;
...
void _gs_slot_params(GSCmd* cmd)
{
    ...
    if (cmd->response_type == GS_RES_STR || cmd->response_type == GS_RES_STR_OK || cmd->response_type==GS_RES_OK_STR) {
    ...
    while (gs.initialized) {
        ...
        if (gs.mode == GS_MODE_NORMAL) {
            ...
            if (gs.slot) {
                ...
                if (cmd) {
                    ...
                    if (cmd == gs.slot->cmd) {
                        ...
                        if (gs.slot->has_params) {
                            ...
                            if(cmd == GS_CMD_CGACT && memcmp(gs.buffer,"NO CARRIER",10)==0) {
                                _gs_slot_ok();
                            }else {
                    ...
                    if (_gs_check_ok()) {
                        ...
                        if (gs.slot->has_params == gs.slot->params && gs.slot->cmd->response_type !=GS_RES_OK_STR) {
                            _gs_slot_ok();
                        } else {
                            if (gs.slot->cmd->id == GS_CMD_CMGL || gs.slot->cmd->id == GS_CMD_QENG) {
                                //variable args
                                _gs_slot_ok();
                            } else if(gs.slot->cmd->id == GS_CMD_QIDNSGIP) {
                                printf("OK for DNSGIP\n");
                            } else if(gs.slot->cmd->id == GS_CMD_QIOPEN) {
                                printf("OK for QIOPEN\n");
                            } else if(gs.slot->cmd->id == GS_CMD_QIRD || gs.slot->cmd->id == GS_CMD_QSSLRECV) {
                                printf("OK for QIRD/SSL\n");
                                gs.mode = GS_MODE_BUFFER;
                                _gs_slot_ok();
                            }else{
                        ...
                        if(gs.slot->cmd->id == GS_CMD_QIRD || gs.slot->cmd->id == GS_CMD_QSSLRECV) {
                            printf("ERROR for QIRD/SSL\n");
                            gs.mode = GS_MODE_BUFFER;
                        }
                        _gs_slot_error();
                    } else if (_gs_check_pdpdeact_urc()) {
                        printf("PDP HALTED\n");
                        gs.pdp_halted = 1;
                        _gs_socket_close_all();
                    } else if (_gs_check_closed_urc()) {
                        int32_t sock_id;
                        printf("handling closed URC\n");
                        if (_gs_parse_command_arguments(gs.buffer,gs.buffer+3,"i",&sock_id) == 1){
                            printf("Closing socket %d\n",sock_id);
                            _gs_socket_closing(sock_id);
                        }
                    } else if (gs.slot->cmd->id==GS_CMD_QIDNSGIP){
                        printf("parsing dns\n");
                        _gs_slot_params(gs.slot->cmd);
                        _gs_slot_ok();
                    } else if (gs.slot->cmd->id==GS_CMD_QIOPEN || gs.slot->cmd->id==GS_CMD_QICLOSE || gs.slot->cmd->id==GS_CMD_QSSLCLOSE){
                        printf("parsing qiopen/close\n");
                        if(gs.buffer[0]>='0' && gs.buffer[0]<='9'){
                            _gs_slot_params(gs.slot->cmd);
                            _gs_slot_ok();
                        }
                        else if (memcmp(gs.buffer, "CLOSE OK", 8) == 0){
                            //not needed: _gs_slot_params(gs.slot->cmd);
                            _gs_slot_ok();
                        }
                        ...
                        if (gs.slot->cmd->id == GS_CMD_QSECWRITE && memcmp(gs.buffer, "CONNECT", 7) == 0) {
                            // go in buffer mode
                            gs.mode = GS_MODE_BUFFER;
                ...
                // we have no slot
                if (_gs_check_pdpdeact_urc()) {
                    printf("PDP HALTED out of slot\n");
                    gs.pdp_halted = 1;
                    _gs_socket_close_all();
                } else if (_gs_check_closed_urc()) {
                    int32_t sock_id;
                    printf("handling closed URC out of slot\n");
                    if (_gs_parse_command_arguments(gs.buffer,gs.buffer+3,"i",&sock_id) == 1){
                        printf("Closing socket %d\n",sock_id);
                        _gs_socket_closing(sock_id);
                    }
...
int _gs_file_delete(uint8_t* filename, int namelen)
{
    ...
    slot = _gs_acquire_slot(GS_CMD_QSECDEL, NULL, 0, GS_TIMEOUT, 0);
    _gs_send_at(GS_CMD_QSECDEL, "=\"s\"", filename, namelen);
    _gs_wait_for_slot();
    res = slot->err;
...
int _gs_file_upload(uint8_t* filename, int namelen, uint8_t* content, int len)
{
    ...
    slot = _gs_acquire_slot(GS_CMD_QSECWRITE, NULL, 64, GS_TIMEOUT * 60, 1);
    _gs_send_at(GS_CMD_QSECWRITE, "=\"s\",i,5,0", filename, namelen, len);
    printf("WAIT\n");
...
int _gs_ssl_cfg(int op, int ctx, int val)
{
    ...
    case 5:
        ...
        break;
    case 6:
        _gs_send_at(GS_CMD_QSSLCFG, "=\"s\",i", "ignorertctime", 13, val); //select date check
        break;
    case 7:
...
{
    // workaround modem halting TCPIP stack on PDP loss
    if (gs.attached && gs.pdp_halted)
        _gs_restart_pdp_tcpip();
...
int _gs_socket_bind(int id, struct sockaddr_in* addr)
{
    ...
    if (sock->proto == 17) {
        ...
        _gs_send_at(GS_CMD_QIOPEN, "=i,\"UDP SERVICE\",\"127.0.0.1\",0,i,0", id, OAL_GET_NETPORT(addr->sin_port));
    }
    _gs_wait_for_slot();
    if (slot->err) {
        res = -1;
...
int _gs_socket_connect(int id, struct sockaddr_in *addr){
    ...
    if (sock->secure) {
        ...
        if (sock->proto == 6) {
            _gs_send_at(GS_CMD_QSSLOPEN, "=i,i,\"s\",i,0", id, id, saddr, saddrlen, OAL_GET_NETPORT(addr->sin_port));
        }
        //NO DTLS!!
        // else {
        ...
        slot = _gs_acquire_slot(GS_CMD_QIOPEN, NULL, MAX_CMD, GS_TIMEOUT * 60 * 3, 1);
        if (sock->proto == 6) {
            _gs_send_at(GS_CMD_QIOPEN, "=i,\"TCP\",\"s\",i,0,0", id, saddr, saddrlen, OAL_GET_NETPORT(addr->sin_port));
        } else {
            //udp
            _gs_send_at(GS_CMD_QIOPEN, "=i,\"UDP\",\"s\",i,0,0", id, saddr, saddrlen, OAL_GET_NETPORT(addr->sin_port));
        }
    }
    _gs_wait_for_slot();
    if (slot->err) {
        ...
        if(!sock->secure) {
            uint8_t *s0=NULL;
            int l0;
            int rr = _gs_parse_command_arguments(slot->resp, slot->eresp, "s", &s0, &l0);
            res=-1;
            if(rr!=1) {
                res = -1;
            } else if (s0) {
                res = memcmp(s0+1,", CONNECT OK",l0-1)==0;
                _gs_socket_opened(id,1);
            }
        }
    }
    _gs_release_slot(slot);
...
int _gs_do_close(int id)
{
    ...
    if (sock->secure) {
        ...
        _gs_send_at(GS_CMD_QSSLCLOSE, "=i", id);
    } else {
        slot = _gs_acquire_slot(GS_CMD_QICLOSE, NULL, 0, GS_TIMEOUT * 15, 1);
        _gs_send_at(GS_CMD_QICLOSE, "=i", id);
    }
    _gs_wait_for_slot();
    if (slot->err) {
        res = -1;
...
int _gs_socket_isalive(int id)
{
    ...
    if (sock->to_be_closed) {
        ...
        if (sock->secure) {
            ...
            slot = _gs_acquire_slot(GS_CMD_QISACK, NULL, 32, GS_TIMEOUT * 10, 1);
            _gs_send_at(GS_CMD_QISACK, "=i", id);
            _gs_wait_for_slot();
            if (!slot->err) {
...
recv_from_buf:
    ...
    if (res >= 0 && len > res && inbuf==0) {
        ...
        //we need to check availability again because inbuf==0 but there might be data to be read
...
//WATCH OUT: the following 2 lines were commented out
        ...
        res = ERR_CLSD;
            ...
            if (sock->secure){
                ...
                _gs_send_at(GS_CMD_QSSLRECV, "=0,i,i", id,MAX_SOCK_RX_LEN);
            } else {
                slot = _gs_acquire_slot(GS_CMD_QIRD, NULL, 64, GS_TIMEOUT * 10, 1);
                _gs_send_at(GS_CMD_QIRD, "=0,1,i,i", id,MAX_SOCK_RX_LEN);
            }
            printf("AVAIL %x %x\n",slot->resp,slot->eresp);

            if (!_gs_wait_for_buffer_mode()) {
                //oops, timeout
                res = ERR_TIMEOUT;
            }
            if(slot->resp != slot->eresp) {
                if (_gs_parse_command_arguments(slot->resp, slot->eresp, "ssi", &s0,&l0,&s1,&l1,&rd) == 3) {
                    if (rd>0) {
                        //read n char
                        sock->head=0;
                        sock->len=0;
                        printf("reading sock buf %i\n",rd);
...
int _gs_resolve(uint8_t* url, int len, uint8_t* addr)
{
    ...
    slot = _gs_acquire_slot(GS_CMD_QIDNSGIP, NULL, MAX_CMD, GS_TIMEOUT * 60, 1);
    _gs_send_at(GS_CMD_QIDNSGIP, "=\"s\"", url, len);
    _gs_wait_for_slot();
    if (slot->err) {
        printf("SLOT ERR\n");
        res = -1;
        ...
        if(rr!=1) {
            ...
            res = MIN(15, l0);
            memcpy(addr, s0, res);
            vosThSleep(TIME_U(100,MILLIS)); //sleep a bit to discard other IP replies
        }
    }
    _gs_release_slot(slot);
    return res;
...
/**
 * @return mobile equipment activity status (+CPAS)
 */
int _gs_get_modem_activity_status(void)
{
    GSSlot* slot;
    int32_t pas = -1; // unexpected/unknown

    slot = _gs_acquire_slot(GS_CMD_CPAS, NULL, 64, GS_TIMEOUT, 1);
    _gs_send_at(GS_CMD_CPAS, "");
    _gs_wait_for_slot();
    if (!slot->err) {
        if (_gs_parse_command_arguments(slot->resp, slot->eresp, "i", &pas) != 1) {
            pas = -1; // unexpected/unknown
        }
    }
    _gs_release_slot(slot);
    return pas;
...
int _gs_control_psd(int activate)
{
    ...
    int res;
    if (activate) {
        slot = _gs_acquire_slot(GS_CMD_QIACT, NULL, 0, GS_TIMEOUT * 180, 0);
        _gs_send_at(GS_CMD_QIACT, "");
        _gs_wait_for_slot();
        res = !slot->err;
        _gs_release_slot(slot);
    } else {
        slot = _gs_acquire_slot(GS_CMD_QIDEACT, NULL, 0, GS_TIMEOUT * 60, 0);
        _gs_send_at(GS_CMD_QIDEACT, "");
        _gs_wait_for_slot();
        res = !slot->err;
        if (res) {
            uint8_t *s0;
...
int _gs_configure_psd(uint8_t* apn, int apnlen, uint8_t* username, int ulen, uint8_t* pwd, int pwdlen, int auth)
{
    ...
    _gs_send_at(GS_CMD_QICSGP, "=1,\"s\",\"s\",\"s\"", apn, apnlen, username, ulen, pwd, pwdlen);
    _gs_wait_for_slot();
    res = !slot->err;
    _gs_release_slot(slot);
    if (res) {
        //register TCP/IP stack
        slot = _gs_acquire_slot(GS_CMD_QIREGAPP, NULL, 0, GS_TIMEOUT, 0);
        _gs_send_at(GS_CMD_QIREGAPP, "=\"s\",\"s\",\"s\"", apn, apnlen, username, ulen, pwd, pwdlen);
        _gs_wait_for_slot();
        res = !slot->err;
        _gs_release_slot(slot);
    }
...
// workaround: loss of PDP connection leaves the TCPIP stack in unknown state
// not fully working (some +QSSLCFG fails). Recover by deactivating connection.
void _gs_restart_pdp_tcpip(void)
{
    _gs_control_psd(0);
    gs.pdp_halted = 0;
}
...
int _gs_is_attached(void)
{
    ...
    uint8_t* s0 = NULL;
    GSSlot* slot;
    slot = _gs_acquire_slot(GS_CMD_QILOCIP, NULL, 64, GS_TIMEOUT, 1);
    _gs_send_at(GS_CMD_QILOCIP, "");
    _gs_wait_for_slot();
    status = !slot->err;
    _gs_release_slot(slot);
    gs.attached = status;
...
int _gs_dns(uint8_t* dns)
{
    ...
    uint8_t *s0 = NULL, *s1;
    GSSlot* slot;
    slot = _gs_acquire_slot(GS_CMD_QIDNSCFG, NULL, 64, GS_TIMEOUT, 2);
...
int _gs_cell_info(int* mcc, int* mnc)
{
    ...
    _gs_send_at(GS_CMD_QENG, "?");
    _gs_wait_for_slot();
    if (!slot->err) {
        if (_gs_parse_command_arguments(slot->resp, slot->eresp, "iii", &p0, mcc, mnc) == 3) {
            res = 1;
        }
    }
    _gs_release_slot(slot);
...
int m95_gzsock_close(int sock) {
    return _gs_socket_close(sock);
}

int m95_gzsock_send(int sock, const void *dataptr, size_t size, int flags) {
    int32_t wrt;
    int32_t tsnd;
    int32_t err=ERR_OK;
...
int m95_gzsock_sendto(int sock, const void *dataptr, size_t size, int flags, const struct sockaddr *to, socklen_t tolen) {
    int32_t wrt;
    int32_t tsnd;
    int32_t err=ERR_OK;
    uint8_t *buf = (uint8_t*) dataptr;
...
int m95_gzsock_recv(int sock, void *mem, size_t len, int flags){
    int rb;
    int trec;
...
int m95_gzsock_recvfrom(int sock, void *mem, size_t len, int flags, struct sockaddr *from, socklen_t *fromlen) {
    int rb;
    int trec;
    uint8_t *buf = (uint8_t*) mem;
...
int m95_gzsock_select(int maxfdp1, void *readset, void *writeset, void *exceptset, struct timeval *tv) {
    uint64_t tstart;
    int32_t timeout;
    uint32_t timepast;
...
int m95_gzsock_read(int sock_id, void *mem, size_t len) {
    return m95_gzsock_recv(sock_id, mem, len, 0);
}

int m95_gzsock_write(int sock_id, const void *dataptr, size_t size) {
    return m95_gzsock_send(sock_id, dataptr, size, 0);
}

int m95_gzsock_fcntl(int s, int cmd, int val) {
    if (cmd != F_GETFL) {
        return -1;
    }
    return O_NONBLOCK;
...
int m95_gzsock_shutdown(int s, int how) {
    return 0;
}

int m95_gzsock_getaddrinfo(const char *node, const char* service, const struct addrinfo *hints, struct addrinfo **res) {
    int32_t saddrlen;
    uint8_t saddr[16];
    int ret;
...
void m95_gzsock_freeaddrinfo(struct addrinfo *ai_res) {
    struct addrinfo *p;
...
int m95_gzsock_setsockopt(int sock_id, int level, int optname, const void *optval, socklen_t optlen) {
...
int m95_gzsock_getsockopt(int sock_id, int level, int optname, void *optval, socklen_t *optlen) {
    if (optname != SO_RCVTIMEO)
        return -1;
...
int m95_gzsock_bind(int sock, const struct sockaddr *name, socklen_t namelen){
